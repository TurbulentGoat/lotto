...
def load_historical_data(filename, main_count, supp_count=0):
    ...
    Assumes:
      col1: draw number
      col2: draw date
      Following columns: main numbers first, then supplementary numbers (which may be 'powerball').
    """
    main_numbers = []
    supp_numbers = []
    ...
    try:
        with open(filename, 'r', newline='', encoding='utf-8') as f:
            reader = csv.reader(f)
            header = next(reader)  # Attempt to skip header if present
            for row in reader:
...
def generate_lines(main_count, supp_count, pool_main, pool_supp,
    ...
    - supp_count: Number of supplementary numbers (could be 'powerball') per line
    ...
    - lines_to_generate: how many lines
    - main_weights: weights for main numbers (for weighted sampling)
    - supp_weights: weights for supplementary (for weighted sampling)
...
def read_user_lines_from_csv(filename, game):
    ...
    Format:
      If supp_count > 0:
         "Line #", "Main Numbers" (space-separated), "Supplementary/Powerball" (space-separated)
      If supp_count == 0:
         "Line #", "Main Numbers" (space-separated)
    """
    user_lines = []
    main_count = game["main_count"]
    supp_count = game["supp_count"]  # Now there's no separate "powerball_count"
    ...
    if lines:
        ...
        if save_csv == 'y':
            with open("lines.csv", "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                if supp_count > 0:
                    writer.writerow(["Line #", "Main Numbers", "Supplementary/Powerball"])
                    for idx, (m, s) in enumerate(lines, start=1):
...
def main():
    # Correcting how many main/supplementary balls are drawn for each:
    # Saturday Lotto: 6 main, 2 supp
    # Oz Lotto: 7 main, 3 supp
    # Powerball: 7 main, 1 supp
